...
class gdmaq_emails_log extends d4p_grid {
	...
	private function _render_tag( $tag, $label, $title = '' ) {
		return '<span class="gdmaq-item-tag gdmaq-tag-' . esc_attr( $tag ) . '"' . ( ! empty( $title ) ? ' title="' . $title . '"' : '' ) . '>' . $label . '</span>';
	}

	private function _render_email( $email, $rel ) {
		$search = urlencode( $rel . ' ' . $email->email );

		return '<span class="gdmaq-single-email gdmaq-email-' . esc_attr( $rel ) . '"><a href="' . $this->_self( "s=" . $search ) . '">' . $email->email . '</a></span>';
	}
